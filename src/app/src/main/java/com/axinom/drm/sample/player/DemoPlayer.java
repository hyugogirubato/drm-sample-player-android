...
class DemoPlayer implements TextOutput, IOfflineLicenseManagerListener, Player.Listener {
    ...
    private void playerCreate(Params params) {
        ...
        try {
            ...
            if (drmConfiguration != null) {
                mDrmSessionManager = buildDrmSessionManager(String.valueOf(drmConfiguration.licenseUri), drmConfiguration.licenseRequestHeaders.get("X-AxDRM-Message"));
                ...
                if (mParams.shouldPlayOffline) {
                    ...
                    mOfflineLicenseManager.getLicenseKeys(String.valueOf(Utility.getPlaybackProperties(params.mediaItem).uri));
                }
            }
        ...
        trackSelector.setParameters(new DefaultTrackSelector.Parameters.Builder(mContext).setPreferredTextLanguage("eng"));
        ...
        try {
            mPlayer = new ExoPlayer.Builder(mContext, renderersFactory).setTrackSelector(trackSelector).build();
    ...
    private DataSource.Factory buildDataSourceFactory() {
        ...
        if (mParams.shouldPlayOffline) {
            ...
            if (axDownloadTracker != null) {
                mDownloadRequest = axDownloadTracker.getDownloadRequest(Utility.getPlaybackProperties(mParams.mediaItem).uri);
        ...
        if (mParams.shouldPlayOffline) {
            ...
            if (mFormat == C.CONTENT_TYPE_DASH) {
                if (mDrmSessionManager != null) {
                    return new DashMediaSource.Factory(new DefaultDashChunkSource.Factory(dataSourceFactory), dataSourceFactory).setDrmSessionManagerProvider(unusedMediaItem -> mDrmSessionManager).createMediaSource(mParams.mediaItem);
                } else {
                    return new DashMediaSource.Factory(new DefaultDashChunkSource.Factory(dataSourceFactory), dataSourceFactory).createMediaSource(mParams.mediaItem);
                }
            } else {
                if (mDrmSessionManager != null) {
                    return new HlsMediaSource.Factory(new DefaultHlsDataSourceFactory(dataSourceFactory)).setDrmSessionManagerProvider(unusedMediaItem -> mDrmSessionManager).createMediaSource(mParams.mediaItem);
                } else {
                    return new HlsMediaSource.Factory(new DefaultHlsDataSourceFactory(dataSourceFactory)).createMediaSource(mParams.mediaItem);
                }
            }
        }
    }
    ...
    private DefaultDrmSessionManager buildDrmSessionManager(String licenseUrl, String drmToken) {
        dispatchPlayerLog("Building DrmSessionManager with licenseUrl = [" + licenseUrl + "] and drmToken = [" + drmToken + "]");
        HttpMediaDrmCallback drmCallback = new HttpMediaDrmCallback(licenseUrl, buildHttpDataSourceFactory());
        // Here the license token is attached to license request
        if (drmToken != null) {
            drmCallback.setKeyRequestProperty("X-AxDRM-Message", drmToken);
        }

        return new DefaultDrmSessionManager.Builder().setUuidAndExoMediaDrmProvider(C.WIDEVINE_UUID, FrameworkMediaDrm.DEFAULT_PROVIDER).setMultiSession(true).build(drmCallback);
    ...
    private void startPlayerPrepare() {
        ...
        MediaSource mediaSource = buildMediaSource(mContext, Utility.getPlaybackProperties(mParams.mediaItem).uri);
        if (mediaSource != null) {
    ...
    public void onNoOfflineLicenseFound(String manifestUrl, int licenseErrorCode) {
        ...
        if (drmConfiguration != null && !TextUtils.isEmpty(drmConfiguration.licenseRequestHeaders.get("X-AxDRM-Message"))) {
        ...
        if (drmMessage != null && drmMessage.persistent) {
            ...
            mOfflineLicenseManager.downloadLicenseWithResult(String.valueOf(drmConfiguration.licenseUri), manifestUrl, drmConfiguration.licenseRequestHeaders.get("X-AxDRM-Message"), true);
        } else {
    ...
    private boolean hasConnection() {
        final ConnectivityManager cm = (ConnectivityManager) mContext.getApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public void onDeletePressed() {
        ...
        AxDownloadTracker axDownloadTracker = AxOfflineManager.getInstance().getDownloadTracker();
        if (axDownloadTracker != null && axDownloadTracker.isDownloaded(String.valueOf(Utility.getPlaybackProperties(mParams.mediaItem).uri))) {
            // License is removed for the selected video
            if (mOfflineLicenseManager != null) {
                mOfflineLicenseManager = new OfflineLicenseManager(mContext);
                mOfflineLicenseManager.setEventListener(this);
                mOfflineLicenseManager.releaseLicense(String.valueOf(Utility.getPlaybackProperties(mParams.mediaItem).uri));
            }
            // Removes a download
            DownloadService.sendRemoveDownload(mContext, AxDownloadService.class, axDownloadTracker.getDownloadRequest(Utility.getPlaybackProperties(mParams.mediaItem).uri).id, false);
        }
    }
}
