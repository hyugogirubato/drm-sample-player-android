...
public class DrmUtils {
    ...
    public static DrmMessage parseDrmString(String drmMessageString) {
        try {
            String token = drmMessageString.substring(drmMessageString.indexOf(".") + 1, drmMessageString.lastIndexOf("."));
            ...
            if (jObject.has("message")) {
                ...
                drmMessage.persistent = jMessage.has("persistent") && jMessage.getBoolean("persistent") || jMessage.has("license") && jMessage.getJSONObject("license").has("allow_persistence") && jMessage.getJSONObject("license").getBoolean("allow_persistence");
                drmMessage.keysBasedOnRequest = jMessage.has("keys_based_on_request") && jMessage.getBoolean("keys_based_on_request");
            }
            return drmMessage;
        } catch (Exception e) {
    ...
    public static SchemeData getSchemeData(SchemeData[] drmInitData, UUID uuid) {
        ...
        if (C.WIDEVINE_UUID.equals(uuid)) {
            for (int i = 0; i < matchingSchemeDatas.size(); i++) {
                ...
                int version = matchingSchemeData.hasData() ? PsshAtomUtils.parseVersion(matchingSchemeData.data) : -1;
    ...
    public static String getSchemeMimeType(SchemeData data, UUID uuid) {
        ...
        if (Build.VERSION.SDK_INT < 26 && C.CLEARKEY_UUID.equals(uuid) && ("video/mp4".equals(schemeMimeType) || "audio/mp4".equals(schemeMimeType))) {
